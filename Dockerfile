...
RUN --mount=type=cache,id=s/61f5be73-a368-437c-bd6f-b76ceaa99048-/root/.local/share/pnpm/store,target=/root/.local/share/pnpm/store,sharing=locked \
...
RUN --mount=type=cache,id=s/61f5be73-a368-437c-bd6f-b76ceaa99048-/var/cache/apt,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=s/61f5be73-a368-437c-bd6f-b76ceaa99048-/var/lib/apt,target=/var/lib/apt,sharing=locked \
    apt-get update && \
...
COPY --from=dependency-inputs /app/scripts/docker/verify-fs-safe-native.mjs /tmp/verify-fs-safe-native.mjs
RUN node /tmp/verify-fs-safe-native.mjs --package-root /app --mode require
